...
function save_item(){
	...
	if($id){
        ...
        if($pdf = upload_image("file_pdf", _format_duoitailieu, _upload_files,$file_name_pdf))
        {
            $pdf = upload_image("file_pdf", _format_duoitailieu, _upload_files,$file_name_pdf);
            $data['file'] = $pdf;
            if($d->num_rows()>0){
				$row = $d->fetch_array();
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_tintuc,$file_name))
		{
			...
			if($_POST['type']=='dichvu')	
			{
				$data['thumb'] = create_thumb($data['photo'], 22, 22, _upload_tintuc,$file_name,2);
			}
			else
			{
				$data['thumb'] = create_thumb($data['photo'], 170, 130, _upload_tintuc,$file_name,1);			

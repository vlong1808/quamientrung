...
<form name="supplier" id="validate" class="form" action="index.php?com=news&act=save&p=<?=$_REQUEST['p']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
     ...
     <div class="widget">
       ...
       <?php if($_GET['type']=='tintuc' or $_GET['type']=='khuyenmai' or $_GET['type']=='tuvan') { ?>
        <div class="formRow">
		...
		<?php }} ?>
      <?php if($_REQUEST['type']=='bangia') { ?>
        <div class="formRow">
			<label>Tải file:</label>
			<div class="formRight">
            	<input type="file" id="file_pdf" name="file_pdf" />
				<img src="./images/question-button.png" alt="Upload file" class="icon_question tipS" original-title="Tải file (*.PDF)">
				<div class="note"> only .pdf  <?=_format_duoitailieu?> </div>
			</div>
			<div class="clear"></div>
		</div>
         <?php if($_GET['act']=='edit'){?>
		<div class="formRow">
			...
			<div class="mt10">File: <?=$item['file']?></div>
    ...
    <?php }?>
 	...
 	<div class="formRow">
       ...
       <div id="content_lang_<?=$key?>" class="tab_content">        
			...
			<?php if($_GET['type']=='tintuc' or $_GET['type']=='dichvu') { ?>
        <div class="formRow">
